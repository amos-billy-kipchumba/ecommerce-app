...
const CreateBrand = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        const response = await api.create('/brands', formData);
        if(response.message === "Brand added") {
      ...
      <div className="bg-white rounded-lg shadow max-w-4xl">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex flex-col gap-4 lg:flex-row justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800">Create Brand</h2>
              <p className="text-sm text-slate-500">Add a new brand to the system</p>
            </div>
          </div>
