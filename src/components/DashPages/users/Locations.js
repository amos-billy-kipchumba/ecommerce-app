...
const Locations = () => {
  ...
  useEffect(() => {
    const getData = async (page) => {
      try {
        let response = await api.read(`/locations?page=${page}&query=${searchQuery}`);
        if (response.data) {
  ...
  useEffect(() => {
    ...
    if(searchQuery !== ""){
      const getData = async () => {
        try {
          let response = await api.read(`/search/location?searchTerm=${searchQuery}`);
          if (response.data) {
            setLocations(response.data)
          } else {
            toast.error("Failure to get locations"); 
          }
        } catch (error) {
          toast.error('Error: ' + error);
        }
    ...
    <div className="max-w-8xl min-h-full pt-28 mx-auto p-4 bg-gray-50">
      <div className="bg-white rounded-lg shadow">
        <div className="px-6 py-5 border-b border-slate-200 text-left">
          ...
          <p className="text-sm text-slate-500 mt-1">Overview of all locations</p>
        </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 text-slate-600">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium">Street</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200">
                {locations.map((location) => (
                  <tr key={location.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-800">{location.area}</td>
                    <td className="px-6 py-4 font-medium text-slate-800">{location.building}</td>
                    <td className="px-6 py-4 font-medium text-slate-800">{location.street}</td>
                  </tr>
                ))}
              </tbody>
            </table>
