...
const Brands = () => {
  ...
  useEffect(() => {
    const getData = async (page) => {
      try {
        let response = await api.read(`/brands?page=${page}&query=${searchQuery}`);
        if (response.data) {
  ...
  useEffect(() => {
    ...
    if(searchQuery !== ""){
      const getData = async () => {
        try {
          let response = await api.read(`/search/brand?searchTerm=${searchQuery}`);
          if (response.data) {
            setBrands(response.data)
          } else {
    ...
    <div className="max-w-8xl min-h-full pt-28 mx-auto p-4 bg-gray-50">
      <div className="bg-white rounded-lg shadow">
        <div className="px-6 py-5 border-b border-slate-200 text-left">
          ...
          <p className="text-sm text-slate-500 mt-1">Overview of all brands</p>
        </div>
